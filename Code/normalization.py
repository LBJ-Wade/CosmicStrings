...
for i in range(10):
    for j in range(10):
        for k in range(10):
            tCube[i, j, k] = i + 10*j + 100*k

"""

def normalizeFFT(completeArray, centeredCube):
    fftArray = np.fft.fftn(completeArray)
    fftCube = np.fft.fftn(centeredCube)
    normCube = np.divide(fftArray,fftCube)
